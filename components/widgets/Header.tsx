...
const Header = () => {
  ...
  return (
    <header
      ...
      } ${isToggleMenuOpen ? "bg-white dark:bg-backgroundDark" : ""}`}
      id="header"
    >
      <div className="mx-auto w-full max-w-7xl py-2 px-4 md:flex md:justify-between md:py-1 md:px-10">
        ...
        <nav
          className={`${isToggleMenuOpen ? "block" : "hidden"} flex justify-center h-screen md:h-auto md:w-full ${
            position === "right"
              ? "justify-end"
              : position === "left"
                ? "justify-start"
                : "justify-center"
          } w-auto overflow-y-auto dark:text-slate-200 md:mx-5 md:flex md:items-center md:overflow-visible`}
              ...
              links.map(({ label, href, icon: Icon, links }, index) => (
                ...
                >
                  {links && links.length ? (
                    <>
                      <Link
                        className="flex items-center px-4 py-3 font-medium transition hover:text-gray-900 dark:hover:text-gray-700"
                        onClick={() => handleDropdownOnClick(index)}
                        onMouseEnter={() => handleDropdownOnClick(index)}
                        aria-expanded={isDropdownOpen[index]}
                        aria-controls={`item-dropdown-${index}`}
                        aria-label={`item-dropdown-${index}`}
                        href={href as string}
                      >
                        {t(label)}{" "}
                        {Icon && (
                      ...
                      <ul
                        ...
                        onMouseLeave={() => handleCloseDropdownOnClick(index)}
                      >
                        {links.map(({ label: label2, href: href2 }, index2) => (
                          <li key={`item-sublink-${index2}`}>
                            <Link
                              className="whitespace-no-wrap block py-2 px-5 first:rounded-t last:rounded-b dark:hover:bg-gray-700 md:hover:bg-gray-200 transition-background"
